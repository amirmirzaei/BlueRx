...
public class DeviceConnector {
    ...
    public DeviceConnector(BluetoothDevice device, Handler handler) {
        ...
        connectedDevice = BluetoothAdapter.getDefaultAdapter().getRemoteDevice(device.getAddress());
    ...
    private synchronized void setState(ConnectionState state) {
        ...
        mState = state;
        mHandler.obtainMessage(MessageState.STATE_CHANGE.id, -1, -1, state.id + "").sendToTarget();
    }
    // ==========================================================================
    ...
    public synchronized void connected(BluetoothSocket socket) {
        ...
        Message msg = mHandler.obtainMessage(MessageState.DEVICE_NAME.id, deviceName);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(MessageState.TOAST.id);
    ...
    public enum MessageState {
        ...
        @NotNull
        public static MessageState findById(int id) {
            for (MessageState val : MessageState.values()) {
                if (val.id == id)
                    return val;
            }
    ...
    enum ConnectionState {
        CONNECTING(0), CONNECTED(1), DISCONNECTED(2), CONNECTION_LOST(3), CONNECTION_FAILED(4);
        ...
        public static ConnectionState findById(int id) {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true && isCanceled == false) {
                try {
                    ...
                    if (readed.contains(delimiter)) {
                        mHandler.obtainMessage(MessageState.READ.id, bytes, -1, readMessage.toString()).sendToTarget();
        ...
        public void writeData(byte[] chunk) {
            ...
            try {
                ...
                mHandler.obtainMessage(MessageState.WRITE.id, -1, -1, chunk).sendToTarget();
            } catch (IOException e) {
        ...
        public void write(byte command) {
            ...
            try {
                ...
                // Share the sent message back to the UI Activity
                mHandler.obtainMessage(MessageState.WRITE.id, -1, -1, buffer).sendToTarget();
            } catch (IOException e) {
                if (D) Log.e(TAG, "Exception during write", e);
            }
        }
